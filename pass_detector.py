...
class PassDetector:
    ...
    def __init__(
        ...
        # NEW: cooldown knobs (to avoid repeated passes)
        ...
        track_cooldown_sec: float = 1.5,    # minimum time between passes from same track_id
    ):
    ...
    def update(self, tracks, now: float, frame_w: int, frame_h: int):
        ...
        # cleanup old cooldown entries (keeps dicts from growing)
    ...
    def _cooldown_ok(self, st: TrackPassState, now: float) -> bool:
        ...
        # per-track cooldown (covers track flapping / re-detect)
    ...
    def _gc_cooldowns(self, now: float):
        # remove very old entries so dicts don't grow forever
